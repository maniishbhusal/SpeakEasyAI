    ...
    recognition.onresult = (event) => {
      ...
      for (let i = event.resultIndex; i < event.results.length; i++) {
        ...
        if (event.results[i].isFinal) {
          ...
          console.log(
            `✅ Final (${Math.round(confidence * 100)}%):`,
            transcript
          );
        } else {
...
function stopRecording() {
  ...
  const doc = parser.parseFromString(finalTranscript, "text/html");
  console.log(doc)
  const plainText = doc.body.textContent || "";

  // Before making the fetch request, add a check:
  if (!plainText.trim()) {
    console.log("Empty transcript, not sending to backend");
    updateStatus("warning", "No text detected to improve");
    return;
  }

  console.log("plain text..." + plainText)

  fetch("/api/improve-transcript/", {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
      "X-CSRFToken": getCSRFToken(), // Add your CSRF logic
    },
    body: JSON.stringify({
    ...
    .then((res) => res.json())
    .then((data) => {
      console.log("🧠 Enhanced Transcript:", data.improved_text);
      // Optionally show enhanced text somewhere
    })
...
function updateTranscriptionDisplay(interimText) {
  ...
  content += `
        <div class="final-transcript">
            ${
              finalTranscript ||
              '<p class="text-gray-500">No transcription yet</p>'
            }
        </div>
    `;

  console.log("finall transcript:   ", finalTranscript);
